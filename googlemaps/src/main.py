from src.utils import get_coordinates, get_places, clean_data
import pandas as pd
...
def procesar_codigos_postales(codigos, tipo_negocio=None):
    ...
    for cp in codigos:
        try:
            print(f"Procesando código postal: {cp}")
            lat, lng = get_coordinates(cp)
            lugares = get_places(lat, lng, business_type=tipo_negocio)
            df_limpio = clean_data(lugares, cp)
            todos_los_locales = pd.concat([todos_los_locales, df_limpio], ignore_index=True)
            print(f"✅ {len(df_limpio)} locales encontrados en {cp}")
...
if __name__ == '__main__':
    ...
    resultado = procesar_codigos_postales(codigos_postales, tipo_negocio=tipo)
